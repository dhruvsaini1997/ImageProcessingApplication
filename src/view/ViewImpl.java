...
public class ViewImpl extends JFrame implements IView {
  ...
  public void addFeatures(Features features) throws IllegalArgumentException {
    ...
    compressionButton.addActionListener(evt -> {
          String compressionValue = compressionText.getText().trim();
          if (!compressionValue.isEmpty()) {
            features.compress(Integer.parseInt(compressionValue), this.currentImage);
            compressionText.setText("0");
          } else {
            showError("Values cannot be blank", errorC);
          }
        }
    );

    brightnessButton.addActionListener(evt -> {
      String brightnessValue = brightnessText.getText().trim();
      if (!brightnessValue.isEmpty()) {
        ...
        showError("Values cannot be blank", errorB);
    ...
    levelsAdjustButton.addActionListener(
        evt -> {
          ...
          String b = levelsAdjustTextB.getText().trim();
          String m = levelsAdjustTextM.getText().trim();
          String w = levelsAdjustTextW.getText().trim();

          if (!b.isEmpty() && !m.isEmpty() && !w.isEmpty()) {
            ...
            showError("Values cannot be blank", errorLa);
          }
